...
Class GymAjaxController extends AppController
{
	...
	public function gymWorkoutData()
	{
		...
		$sel_date=date("m-d-Y",strtotime($this->request->data['sel_date']));
	...
	public function getWorkoutDates()
	{
		...
		if(!empty($data))
		{
			...
			{
				$wid[] = $rec["id"];			
				$date_rages[$x]["start_date"]=$rec["start_date"]->format("m-d-Y");
				$date_rages[$x]["end_date"]=$rec["end_date"]->format("m-d-Y");
			...
			foreach($date_rages as $period)
			{
				...
				if($start->format("m-d-Y") == $end->format("m-d-Y")) //IF start date and end date is same.than also add in array.
				{
					$day = date('l',strtotime($start->format("m-d-Y")));	
	...
	public function viewGroupMember($gid)
	{
		...
		if(!empty($data))
		{
			...
			{
				echo "<tr>
						<td><img src='".$this->request->base ."/upload/{$row['image']}' class='membership-img img-circle'/></td>
	...
	public function viewInvoice($mp_id)
	{
		...
		$sys_data[0]["gym_logo"] = (!empty($sys_data[0]["gym_logo"])) ? $this->request->base . "/upload/".  $sys_data[0]["gym_logo"] : $this->request->base . "/img/Thumbnail-img.png";
			...
			<tbody>
				<tr>
					...
					<td align="right" width="24%">
						<h5><?php $issue_date=$data[0]['created_date']->format("m-d-Y");
			...
			<tbody>
				<tr>
					...
					<td align="<?php echo $float_r;?>" width="24%">
						<h5><?php $issue_date=$data[0]['invoice_date']->format("m-d-Y");
							$issue_date= date($sys_data[0]['date_format'],strtotime($issue_date));
							echo __('Issue Date')." : ". $issue_date;?></h5>
