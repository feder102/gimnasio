...
{  
	...
	public function assignWorkout()
    {
		...
		{  			
			...
			$insert["start_date"] =date("m-d-Y",strtotime($this->request->data["start_date"]));;
			$insert["end_date"] =date("m-d-Y",strtotime($this->request->data["end_date"]));
			$insert["end_date"] =date("m-d-Y",strtotime($this->request->data["end_date"]));
			$insert["end_date"] =date("m-d-Y",strtotime($this->request->data["end_date"]));
			$insert["created_date"] = date("m-d-Y");
			$insert["created_by"] = $session["id"];
			$insert["direct_assign"] = 0;
			
			$this->request->data["start_date"] = date("m-d-Y",strtotime($this->request->data["start_date"]));
			$this->request->data["end_date"] = date("m-d-Y",strtotime($this->request->data["end_date"]));
			...
			{				
				foreach($value['day'] as $day)
				{
					...
					{	$row=array();$workout_data = array();
						...
						$workout_data['created_date'] = date("m-d-Y");
		...
		{ 			
			...
			{ 
				if($k == "GymWorkoutData"){
					...
					if($wid != "")
					{
						$work_outdata[$wid]["start_date"]= $value["start_date"]->format("m-d-Y");							
						$work_outdata[$wid]["end_date"]= $value["end_date"]->format("m-d-Y");							
						$work_outdata[$wid][]=$v;							
					}
				}				
			}			
