...
{     
	...
	public function addWorkout()
    {
		...
		{ 
			...
			$this->request->data["record_date"] = date("m-d-Y",strtotime($this->request->data["record_date"]));
		...
		else if($this->request->is("post") && isset($this->request->data["new_data"]) && !isset($this->request->data["edit"]))
		{
			...
			$assign_data["created_date"]= date("m-d-Y");
	...
	public function addMeasurment($id = null,$type = null)
    {
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				$this->request->data["created_by"]= $session["id"];
				$this->request->data["created_date"]= date("m-d-Y");
				$this->request->data["result_date"]= date("m-d-Y",strtotime($this->request->data["result_date"]));
    ...
    {		
		...
		foreach($user_data as $data)
		{
			...
			$start_date = new \Datetime($data["start_date"]->format("m-d-Y"));
			$end_date =  new \Datetime($data["end_date"]->format("m-d-Y"));			
			$date = $start_date;
		...
		*/
		...
		{
			$wid = $date["id"];
			$date_array[$wid]=$date["record_date"]->format("m-d-Y");
		}
