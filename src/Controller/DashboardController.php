		...
		if(!empty($reservationdata))
		{
			foreach ($reservationdata as $retrieved_data){
				...
				$cal_array [] = array (
						...
						'start' => $retrieved_data["event_date"]->format("m-d-Y")."T{$start_time}",
						'end' => $retrieved_data["event_date"]->format("m-d-Y")."T{$end_time}",
			...
			foreach ( $birthday_boys as $boys ) {
				...
				{				
					/* $cal_array [] = array (
							...
							'start' =>$boys["birth_date"]->format("m-d-Y"),
							'end' => $boys["birth_date"]->format("m-d-Y"),
		...
		if (! empty ( $all_notice )) {
			foreach ( $all_notice as $notice ) {
				...
				$cal_array[] = array (
						...
						'start' => $notice["start_date"]->format("m-d-Y"),
						'end' => date('Y-m-d',strtotime($notice["end_date"]->format("m-d-Y").' +'.$i.' days')),
						'color' => '#12AFCB'
				);	
