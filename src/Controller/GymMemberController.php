...
Class GymMemberController extends AppController
{
	...
	{	
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				$this->request->data['birth_date'] = date("m-d-Y",strtotime($this->request->data['birth_date']));
				$this->request->data['inquiry_date'] = date("m-d-Y",strtotime($this->request->data['inquiry_date']));
				$this->request->data['trial_end_date'] = date("m-d-Y",strtotime($this->request->data['trial_end_date']));
				if(isset($this->request->data['membership_valid_from']))
				{
					$this->request->data['membership_valid_from'] = date("m-d-Y",strtotime($this->request->data['membership_valid_from']));
				
				}
				if(isset($this->request->data['membership_valid_to']))
				{
					$this->request->data['membership_valid_to'] = date("m-d-Y",strtotime($this->request->data['membership_valid_to']));
				}
				$this->request->data['first_pay_date'] = date("m-d-Y",strtotime($this->request->data['first_pay_date']));
				$this->request->data['created_date'] = date("m-d-Y");
	...
	public function editMember($id)
	{
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				/* $this->request->data['image'] = $image ; */
				$this->request->data['birth_date'] = date("m-d-Y",strtotime($this->request->data['birth_date']));
				$this->request->data['inquiry_date'] = (($this->request->data['inquiry_date'] != '')?date("m-d-Y",strtotime($this->request->data['inquiry_date'])):'');
				$this->request->data['trial_end_date'] = (($this->request->data['trial_end_date'] != '')?date("m-d-Y",strtotime($this->request->data['trial_end_date'])):'');
				if(isset($this->request->data['membership_valid_from']))
				{
					$this->request->data['membership_valid_from'] = date("m-d-Y",strtotime($this->request->data['membership_valid_from']));
				}
	...
	{	
		...
		{ 			
			...
			$s_date = date("m-d-Y",strtotime($this->request->data["sdate"]));
			$e_date = date("m-d-Y",strtotime($this->request->data["edate"]));			
