		...
		<div class="box-body">							    
			...
			?>
				...
				<div class="form-group col-md-3">
					...
					<input type="text" name="sdate" class="form-control sdate datepick" value="<?php echo ($view)?$s_date:date("m-d-Y");?>">
				</div>
				<div class="form-group col-md-3">
					<label for="exam_id"><?php echo __("End Date");?></label>
					<input type="text" name="edate" class="form-control edate datepick" value="<?php echo ($view)?$e_date:date("m-d-Y");?>">
				</div>
			...
			<tbody>
				...
				if(!empty($data))
				{
					foreach($data as $row)
					{
						$date = $row["attendance_date"]->format("m-d-Y");?>				
						<tr>
